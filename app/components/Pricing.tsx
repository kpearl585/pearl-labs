    ...
    <section className="py-40 px-6" id="pricing"
      ...
      <div className="max-w-6xl mx-auto">
        <motion.div className="mb-20 text-center"
          ...
          <p className="text-xs font-bold uppercase tracking-[0.25em] mb-5" style={{ color: 'rgba(255,255,255,0.25)' }}>Transparent pricing</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
          {plans.map((p, i) => (
            ...
            >
              ...
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(255,255,255,0.4)' }}>{p.name}</h3>
                <div className="flex items-baseline gap-2 mb-1">
                  <span className="font-display font-bold text-white" style={{ fontSize: '2.25rem' }}>{p.price}</span>
                  <span className="text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>{p.period}</span>
                </div>
                <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>{p.tagline}</p>
              </div>
              ...
              <a href={p.href} target="_blank" rel="noopener noreferrer"
                className="w-full text-center py-3.5 rounded-xl text-sm font-semibold transition-all duration-300"
                style={p.featured ? {
